...
-- (c) 장르별 미디어 타입 종류 나열
select g.Name as 장르, group_concat(DISTINCT m.Name) as 미디어타입목록
from tracks t
...
-- 테이블 invoices
select InvoiceId, CustomerId, Total from invoices where Total > (select avg(Total) from invoices) ORDER BY Total desc;
...
-- 총매출 구하기
-- 고객ID, 고객 이름, 총매출
select sub.CustomerId, c.FirstName || ' ' || c.LastName, sub.총매출 from (
...
-- 출력: Customerid, FirstName + LastName 고객명 as 고객명, 'Has Invoice' / 'No Invoice'
-- HINT: where절 Customer ID 활용해서 서브쿼리
select CustomerId, FirstName || ' ' || LastName as 고객명, 'Has Invoice'
from customers
...
select strftime('%Y', InvoiceDate) as 연도, Round(sum(Total), 2)
from invoices
group by 연도
union all
select '------전체합계', round(sum(Total),1)
from invoices;

-- (b) 장르별 트랙 수 + 전체 합계
-- 테이블명: genres, tracks
select g.Name as 장르별, count(t.TrackId) as 갯수
from genres g
left join tracks t
...
--고객 이름 궁금함, 고객 Id, FristName, LastName
...
inner join (
    ...
    from invoices
    where strftime('%Y', InvoiceDate) = '2010'
) y on c.CustomerId = y.CustomerId
...
-- 집합연산자 : union all
select g.Name as 장르, count(distinct ii.InvoiceId) as 청구서수, sum(ii.Quantity) as 판매수량, round(sum(ii.UnitPrice * ii.Quantity),1) as 매출
from invoice_items ii
inner join tracks t on ii.TrackId = t.TrackId
inner join genres g on t.GenreId = g.GenreId
group by g.Name
union all
select '------전체 합계', count(distinct InvoiceId ), sum(Quantity),round(sum(UnitPrice * Quantity),1)
from invoice_items
