...
select Country, count(*) as count_ct
from customers
group by Country
order by count(*) desc;
...
-- 테이블 : invoices
select CustomerId, count(*) as customer_cnt
from invoices
group by CustomerId
having count(*) >= 7;
...
select customers.FirstName as customer, employees.FirstName as employee
from customers
...
select tracks.Name as track_name, albums.Title as album_title, artists.Name as artists_name
from tracks
...
select artists.Name as artist_name, albums.Title as albums
from artists
left join albums
